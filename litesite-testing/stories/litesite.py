...
Requirements:
    ...
    - all input files you want the script to process must include a 
      YAML frontmatter block containing the tag `litesite:`.
      Files without this tag will be ignored.
...
Optional frontmatter:
    ...
    frontmatter is fine; the script will ignore it. (Modify the script
    if there's a custom tag you'd like to process.) 
...
Optional placeholders:
    ...
    html when inserting placeholders in your files:
...
Tips:
    ...
    self-contained site -- so you could run it in separately in several
    ...
    all PREV_*_PH, NEXT_*_PH and TOC_BLOCK_PH placeholders and using
    the same header and footer files for all pages, or by having
...
from typing import Iterable, List, Dict, Any # type hinting support
import datetime  as dt # enable sorting on date
from pathlib  import Path # system-independent filepath manipulation
import frontmatter # parse input files with YAML frontmatter
import pypandoc as ppd  # convert input files to html
from operator import itemgetter  # sort the Collection
from copy import deepcopy  # avoid function side effects

#   -- USER SETTINGS ---
SITE_NAME = "Grayson's Book of Stories"
LANGUAGE = 'en'  # two-letter iso code
DOMAIN = 'https://gbmj.net/'  # include protocol and end in a slash
PATH_FROM_DOMAIN_TO_HERE = 'litesite-testing/stories/'  # end in a slash
HHF_SUBDIR = 'cmn/'  # where head, header, footer files are; end in a slash
HDFN =  'head.html'
...
OPTS = []  # pandoc options to pass in to conversion
...
def _get_infiles(dir: Path, pattern: str, maxdepth: int = 10) -> Iterable[Path]:
...
def _create_meta_defaults(path: Path) -> dict[str, Any]:
...
def _process_incoming_meta(current: dict[str, Any], new: dict[str, Any]) -> None:
...
# --- testing ---
_testing = False
# uncomment next line to see available values for CONVERT_FROM
#print(ppd.get_pandoc_formats()[1])
# --- end testing ---

if __name__ == '__main__' and not _testing:

    coll: list[tuple[str,str,str,str,Path]] = []
    ...
    for files in _get_infiles(BASEDIR, rf"*.{INFILE_EXT}", MAXDEPTH):
        for filepath in files:
            ...
            if 'litesite' in tags:
                html_body = ( ppd.convert_text(body, 'html', CONVERT_FROM, 
                                               extra_args=OPTS) 
                ...
                if tags['litesite'] == 'collection':
                    coll.append(
                        (m['title'], m['date'], m['blurb'], m['url'], m['path'])
                        )
    ...
    if (infile.exists()):
        ...
        prev = f'<a href="{sorted_meta[-1][idx_url]}">{PREV_ANCHOR_TXT}</a>'
        next = f'<a href="{sorted_meta[0][idx_url]}">{NEXT_ANCHOR_TXT}</a>'
        html_page = (
